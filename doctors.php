...
<body>
    <div class="container">
        ...
        <button class="btn btn-primary btn-custom my-3">
            <a href="add_doc.php" class="btn-custom">Add Doctors</a>
        </button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.update-btn').forEach(button => {
                button.addEventListener('click', function() {
                    window.location.href = `doc_update.php?updateid=${this.getAttribute('data-id')}`;
                });
            });
            ...
            document.querySelectorAll('.delete-btn').forEach(button => {
                button.addEventListener('click', function() {
                    if (confirm('Are you sure you want to delete this doctor?')) {
                        window.location.href = `doc_delete.php?deleteid=${this.getAttribute('data-id')}`;
                    }
                });
            });
        });
